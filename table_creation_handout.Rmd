...
Employer tables are base tables for generating employer characteristics that we will help us answer the question about TANF leavers - whare the characteristics of employers that TANF leavers go on to find stable employmnet? 

We will calculate the following meaures:
...
The form of the table is equivalent to that of the wage tables. However, only those individuals who were employed for three consective quarters, beginning with the exit quarter, are considered _stably employed_ and are in this table and therefore contribute to the aggregate statistics. Individuals who do not have 3 consecutive quarters of employment by the same are filtered out by virtue of the fact that the `CROSS JOIN` keys are `ssn` (person), `fein` (firm) and `job_yr_q` (joined on consecutive year-quarters). 
