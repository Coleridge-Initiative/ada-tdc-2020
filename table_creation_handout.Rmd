...
Two tables were created in the TANF data: `ada_tdc_2020.tanf_2009q1` and `ada_tdc_2020.tanf_2016q4` which contain spells for primary TANF recipients who ended a spell in Q1 2009 or Q4 2016, respectively. The TANF cohort tables originate from the `person_month` table in the `in_fssa` schema. `r margin_note('See [Data Dictionary](https://ada.coleridgeinitiative.org/ada-2019-tdc#data-doc){target="_blank"}')` 

## Form of Table

```{marginfigure, echo=TRUE}
`tanf_total_months` is the sum of `tanf_spell_months` across all spells that the individual had.
```

```{r, echo = F}
df <- data.frame(matrix(nrow = 4, ncol = 9))
colnames(df)<-c("ssn","month","tanf_start","tanf_end","tanf_spell_months","tanf_total_months","county","rep_year","rep_month")

df$ssn <- c("ssnA","ssnB","ssnC","ssnD")
df$month <- c("200901","200902","200901","200903")
df$tanf_start <- c(FALSE, TRUE, FALSE, FALSE)
df$tanf_end <- c(TRUE, TRUE, TRUE, TRUE)
df$tanf_spell_months <- c(4,1,7,13)
df$tanf_total_months <- c(10,1,18,13)
df$county <- c(097, 052,095,173)
df$rep_year <- c(2009,2009,2009,2009)
df$rep_month <- c(1,2,1,3)

kable(df,align=rep('c', 9))
```
...
Employer tables are base tables for generating employer characteristics that we will help us answer the question about TANF leavers - whare the characteristics of employers that TANF leavers go on to find stable employment?

We will calculate the following measures:
...
`tot_employment` and other measures for _all employment_ are calculated in the quarter after exit
          
`full_quarter_employment` and other measures of _stable employment_ are calculated with the quarter after exit as reference quarter
```
...
empl_char_df <- empl_char_df %>% 
  mutate(Description = case_when(
    Column == "fein" ~ "Business Identifier",
    Column == "naics_3_digit" ~ "Industry identifier",
    Column == "year" ~ "year of wage record",
    Column == "quarter" ~ "quarter of wage record",
    Column == "job_yr_q" ~ "year-quarter in date format",
    Column == "tot_employment" ~ "count(ssn) in quarter after exit",
    Column == "tot_earnings" ~ "sum(wages) in quarter after exit",
    Column == "avg_earnings" ~ "sum(wages)/count(ssn) in quarter after exit",
    Column == "bottom_25_earnings" ~ "earnings in quarter after exit, at the 25th percentile",
    Column == "top_25_earnings" ~ "earnings in quarter after exit, at the 75th percentile",
    Column == "full_quarter_employment" ~ "count(ssn) for ssns that were stably employed",
    Column == "full_quarter_earnings" ~ "sum(wages) for wages where the ssns were stably employed",
    Column == "full_quarter_avg_earnings" ~ "sum(wages)/count(ssn) for wages/ssn that were stably employed"
  ))
...
The base tables for employment characteristics are wages from firms in the quarter of exit and 2 quarters post-exit. Base tables are `all_empl_wage_2016q4` and `all_empl_wage_2009q1`.
...
_Inclusion Criteria_    
    ...
    - All firms     
    - For the measures of stable employment, wage records for individuals who were employed in exit quarter + 2 quarters post-exit by the same employer 
...
_Earnings/Employment, all employment, 1st quarter post-exit_
```{marginfigure, echo=TRUE}
Earnings/employment measures are calculated at a point-in-time, giving a snapshot of the firm. The wage records are filtered for the quarter post-exit (Q1 2017).
```
```yaml
...
Using that base table, we calculate the full-quarter employment metrics and update the characteristics table.
...
_Inclusion Criteria_     
    ...
    - Individuals who had three consecutive quarters of employment at the same firm  
    - Looking at stably employed individuals with a reference quarter as the first quarter after exit of our TANF cohorts

The form of the table is equivalent to that of the wage tables. However, only those individuals who were employed for three consecutive quarters, beginning with the exit quarter, are considered _stably employed_ and are in this table and therefore contribute to the aggregate statistics. Individuals who do not have 3 consecutive quarters of employment by the same are filtered out by virtue of the fact that the `CROSS JOIN` keys are `ssn` (person), `fein` (firm) and `job_yr_q` (joined on consecutive year-quarters). 

_Form of the table created with a `CROSS-JOIN`, the base table for employment outcomes_
```{r, echo = F, fig.fullwidth = TRUE}
cross_join_df <- data.frame(matrix(nrow = 4, ncol = 7))
colnames(cross_join_df)<-c("ssn","year","quarter","uiacct","wages","fein","job_yr_q")
...
cross_join_df$job_yr_q<- c("2010-04-01","2010-04-01","2010-04-01","2010-04-01")

kable(cross_join_df,align=rep('c', 7))
```
