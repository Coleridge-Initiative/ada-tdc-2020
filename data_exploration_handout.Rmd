...
1.  How many primary recipients of TANF had a spell ending in 2016 Q4? 2009 Q1?
1.  Does the number of individuals receiving benefits vary by county?
...
<span style="color:green; ">How many primary recipients of TANF had a spell ending in 2009 Q1? 2016 Q4? </span>


We can do this in calculation in SQL (here, with the 2016 Q4 cohort)

```yaml
`r code_snips$code[code_snips$title=="tanf_count_2016q4"]`
```

And we can also do it in R, for the 2009 Q1 cohort. We'll first read in the entire `tanf_2009q1` table.


```yaml
`r code_snips$code[code_snips$title=="read_tanf_2009q1"]`

```

Then, we can count the number of unique individuals using the `unique` and `length` functions from base R.

```yaml
`r code_snips$code[code_snips$title=="tanf_count_2009q1_r"]`

```

<span style="color:green; ">Does the number of individuals receiving benefits vary by county?</span>

We will show how to solve this with SQL and R.

**2016 Q4 Cohort, SQL**

```yaml
`r code_snips$code[code_snips$title=="tanf_count_by_county_2016q4"]`


```
We display the aggregaate counts by sorting on the `count` field using the `order` function from base R. 

```yaml
`r code_snips$code[code_snips$title=="sort_tanf_count_by_county_2016q4"]`


```
**2009 Q1 Cohort, R**

We'll start to use data manipulation functions from the `tidyverse` library. Since we are looking for the number of individuals per region, we can use `SELECT DISTINCT` when we read in the `ssn` and `county` fields.

First, read in the table.

```yaml
`r code_snips$code[code_snips$title=="select_tanf_ssn_county_2016q4"]`

```
Then using functions from `tidyverse`, we do the aggregation. `count` takes as input the grouping variable and returns the row count per group in a new column automatically named `n`. `arrange` is similar to the `order` function we saw before, accepting `-` to sort in descending order. 
...
Explore 2016 Q4 TANF Leavers. How does the distribution we just calculated vary by region? Use `county` field to look at the distribution within a specific county. 


_Solution_
```yaml
`r code_snips$code[code_snips$title=="checkpoint3_solution"]`
...
4b
```yaml
`r code_snips$code[code_snips$notebook == "data_exploration" & code_snips$title=="checkpoint4b_solution"]`

```

<span style="color:green; ">Are we seeing a concentration of lengthy spells in specific regions?</span>

We want to take a better look counties that have a high proportion of "long term stayers" (defined here as being above the 90th percentile value) relative to all individuals in the cohort. We will introduce some common useful functions from the `tidyverse` library. 


First we retrieve the values at the 90th percentile and store it as `val_90`.

```yaml
`r code_snips$code[ code_snips$title=="long_stayers_input"]`

```

Then we read in the cohort
```yaml
`r code_snips$code[ code_snips$title=="read_tanf_2016q4"]`

```
We use `mutate` to create a new column `long_term` as a flag to indicate if the spell was long-term by our definition of being longer than the 90th percentile value. We impose this condition with `ifelse`.

```yaml
`r code_snips$code[ code_snips$title=="create_long_term_df"]`
```
Finally, we use:

    - `count` to count rows by county and long term status with the flag we just created  
    - `spread` to pivot the counts from rows into columns
    - `replace_na` to replace NA values with 0 for counties where there were no rows for a given `long_term_status` value
    - `mutate` to calculaate the percentage of long term stayers to total
    - `arrange` to sort by the percentage
    - `head` to see the first 6 rows. We've used head many times but now we show how it can be used in a chained expression.

```{marginfigure echo = TRUE}
Note that `tidyverse` is comprised of multiple other libraries that we use here like `dplyr` and `tidyr`. We are following the best practice of putting `library::function` here.
```

```yaml
`r code_snips$code[ code_snips$title=="summarize_long_term_df"]`
...
1.  What proportion of TANF leavers from our cohorts had jobs within a year of their exit?
...
<span style="color:green; ">What proportion of TANF leavers from our cohorts had jobs within a year of their exit?</span>

We first created a user-defined function to make it easy to run for multiple cohorts

```yaml
`r code_snips$code[code_snips$title == "employed_within_year_function"]`

```

And then ran it with our two cohorts

```yaml
`r code_snips$code[code_snips$title == "run_employed_within_year_function"]`
